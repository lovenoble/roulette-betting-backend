...
export class User implements UserServer {
  ...
  public async verifyToken(
    ...
    try {
      ...
      if (!publicAddress) {
        return callback(new ServiceError(status.PERMISSION_DENIED, 'Token is invalid'), null)
      }
      ...
      if (!doesExist) {
        return callback(new ServiceError(status.PERMISSION_DENIED, 'User does not exist'), null)
      }
  ...
  public async verifySignature(
    ...
    try {
      ...
      return callback(new ServiceError(status.PERMISSION_DENIED, 'Signature is invalid'), null)
    } catch (err) {
  ...
  public async setUserData(
    ...
    try {
      ...
      const { token, username, email, colorTheme: _colorTheme } = call.request
