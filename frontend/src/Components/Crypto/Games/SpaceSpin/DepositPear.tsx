...
const SDepositBtn = styled(motion.button)`
    border: none;
...
const SDepositModal = styled.div`
    ...
    box-shadow: 0px 0px 100px 40px rgb(38 10 38 / 58%);
    border-radius: 4px;
    > h3 {
        text-align: center;
        margin-bottom: 4px;
...
function DepositPear({ pearTokenContract, provider, publicAddress, pearBalance, depositBalance }: IDepositPear) {
    ...
    const depositPear = useCallback(async () => {
        try {
            ...
            if (Number(inputRef.current.value) > Number(pearBalance))
                return alert(
                    `Not enough PEAR. Please deposit less than ${pearBalance}`
                )
            ...
            const depositResp = await pearTokenContract.depositIntoEscrow(
                safeAmount
            )

            await provider.waitForTransaction(depositResp.hash)
            setLoadingText('Transaction successful.\n Updating state...')
        } catch (err) {
            console.error(err)
            setIsWaiting(false)
        }
    }, [pearTokenContract, provider, pearBalance])
    ...
    return (
        <SDepositPear>
            <button onClick={toggleModal}>Deposit Pear</button>
            <AnimatePresence>
                {showModal && (
                    <Portal>
                        <SDepositOverlay>
