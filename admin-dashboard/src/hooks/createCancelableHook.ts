...
function createCancelableHook<T extends Fn, O>(
    createFn: (fnWrapper: Fn, wait?: number, options?: O) => T & Cancelable,
    depFn: (wait?: number, options?: O) => any[],
) {
    return (fn: T, wait?: number, options?: any, deps?: ReadonlyArray<any>) => {
        ...
        if (options.setCancel) {
            options.setCancel(debouncedFn.current)
        }
